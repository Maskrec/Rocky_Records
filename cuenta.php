...
<body>
    ...
    <main class="contenedor-seccion" style="max-width: 1000px; margin: 40px auto; min-height: 70vh;">
        <div
            style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid var(--color-borde); padding-bottom: 10px; margin-bottom: 35px;">
            <h1 style="font-family: var(--fuente-serif); margin: 0; text-transform: uppercase;">Mi Cuenta</h1>
            <a href="cuenta.php?logout=1" class="boton-explorar"
                style="background-color: #78716c; font-size: 11px; padding: 8px 16px;">CERRAR SESION</a>
        </div>

        <!-- Datos del Usuario -->
        <section style="margin-bottom: 45px;">
            <h2
                style="font-family: var(--fuente-sans); font-size: 18px; font-weight: 800; text-transform: uppercase; margin-bottom: 15px; color: var(--texto-oscuro);">
                Datos Personales</h2>
            <?php
            try {
                ...
                if ($usuario) {
                    ...
                    echo '<tr><th style="padding: 12px; border: 1px solid var(--color-borde); text-align: left; background-color: #c2410c; font-size: 13px; font-weight: 700; width: 30%;">NOMBRE</th><td style="padding: 12px; border: 1px solid var(--color-borde); font-size: 14px;">' . htmlspecialchars($usuario['nombre']) . '</td></tr>';
                    echo '<tr><th style="padding: 12px; border: 1px solid var(--color-borde); text-align: left; background-color: #c2410c; font-size: 13px; font-weight: 700;">EMAIL</th><td style="padding: 12px; border: 1px solid var(--color-borde); font-size: 14px;">' . htmlspecialchars($usuario['email']) . '</td></tr>';
                    echo '<tr><th style="padding: 12px; border: 1px solid var(--color-borde); text-align: left; background-color: #c2410c; font-size: 13px; font-weight: 700;">ROL</th><td style="padding: 12px; border: 1px solid var(--color-borde); font-size: 14px; text-transform: uppercase; font-weight: 700; color: ' . ($usuario['rol'] === 'admin' ? 'var(--color-naranja)' : 'inherit') . ';">' . htmlspecialchars($usuario['rol']) . '</td></tr>';
                    echo '<tr><th style="padding: 12px; border: 1px solid var(--color-borde); text-align: left; background-color: #c2410c; font-size: 13px; font-weight: 700;">FECHA REGISTRO</th><td style="padding: 12px; border: 1px solid var(--color-borde); font-size: 14px;">' . htmlspecialchars($usuario['fecha_registro']) . '</td></tr>';
                    echo '</table>';
