...
if (isset($_GET['eliminar_id'])) {
    try {
        ...
        // 1. Eliminar referencias del producto en el carrito primero
        $stmt_del_cart = $pdo->prepare("DELETE FROM carrito WHERE producto_id = ?");
        $stmt_del_cart->execute([$eliminar_id]);

        // 2. Obtener archivos para borrarlos fisicamente del disco
...
<head>
    ...
    <style>
        ...
        .tabla-productos-admin th, .tabla-productos-admin td {
            padding: 10px 12px;
            border: 1px solid var(--color-borde);
            text-align: left;
            font-size: 13px;
...
<body>
    ...
    <main class="contenedor-seccion">
        <div class="contenedor-admin">
            ...
            <?php if ($es_edicion): ?>
                <h2 style="font-family: var(--fuente-serif); font-size: 24px; border-bottom: 2px solid var(--color-borde); padding-bottom: 10px; margin-bottom: 25px; text-transform: uppercase; color: var(--color-naranja);">Editar Album: "<?php echo htmlspecialchars($prod_edit['titulo']); ?>"</h2>
            <?php else: ?>
                <h2 style="font-family: var(--fuente-serif); font-size: 24px; border-bottom: 2px solid var(--color-borde); padding-bottom: 10px; margin-bottom: 25px; text-transform: uppercase;">Agregar nuevo album a la tienda</h2>
            <?php endif; ?>
            ...
            <form action="guardar_producto.php<?php echo $es_edicion ? '?action=edit&id=' . $prod_edit['id'] : '?action=add'; ?>" method="POST" enctype="multipart/form-data" class="form-admin">
                <div class="form-grupo">
                    <label for="titulo">Titulo del album</label>
                    <input type="text" id="titulo" name="titulo" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['titulo']) : ''; ?>" placeholder="Ej: Abbey Road" required>
                </div>

                <div class="form-grupo">
                    <label for="artista">Nombre del artista / banda</label>
                    <input type="text" id="artista" name="artista" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['artista']) : ''; ?>" placeholder="Ej: The Beatles" required>
                </div>
                ...
                <div class="form-grupo">
                    ...
                    <input type="date" id="Fecha_Publicacion" name="Fecha_Publicacion" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['Fecha_Publicacion'] ?? '') : ''; ?>">
                </div>

                <div class="form-grupo">
                    <label for="precio">Precio (MXN)</label>
                    <input type="number" id="precio" name="precio" step="0.01" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['precio']) : ''; ?>" placeholder="Ej: 499.00" required>
                </div>

                <div class="form-grupo">
                    <label for="precio_oferta">Precio de oferta (Opcional - MXN)</label>
                    <input type="number" id="precio_oferta" name="precio_oferta" step="0.01" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['precio_oferta'] ?? '') : ''; ?>" placeholder="Ej: 399.00">
                </div>

                <div class="form-grupo">
                    <label for="stock">Cantidad en Stock</label>
                    <input type="number" id="stock" name="stock" value="<?php echo $es_edicion ? htmlspecialchars($prod_edit['stock']) : ''; ?>" placeholder="Ej: 10" required>
                </div>

                <div class="form-grupo form-grupo-full">
                    <label for="descripcion">Descripcion del album</label>
                    <textarea id="descripcion" name="descripcion" placeholder="Escribe detalles del album..." rows="4"><?php echo $es_edicion ? htmlspecialchars($prod_edit['descripcion'] ?? '') : ''; ?></textarea>
                </div>

                <div class="form-grupo form-grupo-full" style="border-top: 1px solid var(--color-borde); padding-top: 15px; margin-top: 5px;">
                    <label for="imagen">Imagen de portada <?php echo $es_edicion ? '(Opcional - deja vacio para mantener actual)' : ''; ?></label>
                    <input type="file" id="imagen" name="imagen" accept="image/*" <?php echo $es_edicion ? '' : 'required'; ?>>
                    <?php if ($es_edicion && $prod_edit['imagen_url']): ?>
                        <span style="font-size: 11px; color: var(--texto-atenuado);">Actual: <?php echo htmlspecialchars($prod_edit['imagen_url']); ?></span>
                    <?php endif; ?>
                </div>

                <div class="form-grupo form-grupo-full" style="border-bottom: 1px solid var(--color-borde); padding-bottom: 15px; margin-bottom: 10px;">
                    <label for="audio">Audio Demo MP3 <?php echo $es_edicion ? '(Opcional - deja vacio para mantener actual)' : ''; ?></label>
                    <input type="file" id="audio" name="audio" accept="audio/mpeg" <?php echo $es_edicion ? '' : 'required'; ?>>
                    <?php if ($es_edicion && $prod_edit['demo_url']): ?>
                        <span style="font-size: 11px; color: var(--texto-atenuado);">Actual: <?php echo htmlspecialchars($prod_edit['demo_url']); ?></span>
                    <?php endif; ?>
                </div>

                <div class="form-grupo form-grupo-full" style="flex-direction: row; gap: 10px;">
                    <button type="submit" class="boton-explorar" style="flex-grow: 1; justify-content: center; font-size: 13px;">
                        <?php echo $es_edicion ? 'GUARDAR CAMBIOS ➔' : 'SUBIR A LA TIENDA ➔'; ?>
                    </button>
                    <?php if ($es_edicion): ?>
                        <a href="admin.php" class="boton-explorar" style="background-color: #78716c; text-decoration: none; justify-content: center; font-size: 13px;">CANCELAR</a>
                    <?php endif; ?>
                </div>
            </form>

            <!-- Listado de Productos -->
            <h2 style="font-family: var(--fuente-serif); font-size: 20px; border-bottom: 2px solid var(--color-borde); padding-bottom: 10px; margin-top: 50px; margin-bottom: 20px; text-transform: uppercase;">Inventario de Productos</h2>
