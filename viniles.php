    ...
    <main class="contenedor-seccion">
        ...
        <div class="cabecera-seccion">
            <h2>
                <?php
                if ($genero_nombre !== '') {
                    echo "Viniles de " . htmlspecialchars($genero_nombre);
                } elseif ($col_nombre !== '') {
                    echo "Viniles: " . htmlspecialchars($col_nombre);
                } else {
                    echo "Catalogo de Viniles";
                }
                ?>
            </h2>
        </div>
        ...
        <div class="catalogo">
            ...
            if ($genero_nombre !== '') {
                $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND (genero LIKE ? OR genero = ?) ORDER BY fecha_agregado DESC");
                $stmt->execute(["%$genero_nombre%", $genero_nombre]);
            } elseif (isset($_GET['coleccion_id'])) {
                $c_id = (int)$_GET['coleccion_id'];
                if ($c_id === 1) {
                    // Ediciones Limitadas: stock bajo (1 a 5 unidades)
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND stock > 0 AND stock <= 5 ORDER BY fecha_agregado DESC");
                } elseif ($c_id === 2) {
                    // Los Más Vendidos: stock alto o más de 10 unidades
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND stock > 10 ORDER BY fecha_agregado DESC");
                } elseif ($c_id === 3) {
                    // Descubrimientos: fecha de publicación reciente (desde 2020)
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND Fecha_Publicacion >= '2020-01-01' ORDER BY fecha_agregado DESC");
                } elseif ($c_id === 4) {
                    // TOP 5: los 5 más caros
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' ORDER BY precio DESC LIMIT 5");
                } elseif ($c_id === 5) {
                    // Clásicos: fecha de publicación anterior a 2010
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND Fecha_Publicacion < '2010-01-01' AND Fecha_Publicacion > '1900-01-01' ORDER BY fecha_agregado DESC");
                } elseif ($c_id === 6) {
                    // Poco Conocido: stock igual a 2 o géneros alternativos
                    $stmt = $pdo->prepare("SELECT * FROM productos WHERE formato = 'Vinil' AND (stock = 2 OR genero = 'Ruido negro' OR genero = 'jazz') ORDER BY fecha_agregado DESC");
                } else {
